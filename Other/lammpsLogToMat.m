...
%%
tic
nCarbonsPerChain = 100;%Required
nChains = 136;%Will still work if set to 1, won't preallocate
nTimesteps = 3628;%Will still work if set to 1, won't preallocate

mdData = single(zeros(nTimesteps,nChains,nCarbonsPerChain,3));
stepN = 0;
timesteps = zeros(nTimesteps,1);

fprintf('Beginning read of log file %s\n',fileName)
...
while ~feof(fID)
   ...
   switch currentLine
      case 'ITEM: TIMESTEP'
         ...
         timesteps(stepN) = str2double(fgetl(fID)); %#ok<*SAGROW>
