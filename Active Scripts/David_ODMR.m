...
%Required
p.scanBounds = [3.55 3.7]; %Frequency bounds
...
p.sequenceTimePerDataPoint = 0.25;%Before factoring in forced delay and other pauses
p.maxFailedCollections = 10;
p.nIterations = 1; %Number of iterations of scan to perform
p.timeoutDuration = 5; %How long besfore auto-continue occurs
p.forcedDelayTime = .125; %Time to force pause before (1/2) and after (full) collecting data
p.nDataPointDeviationTolerance = 1e-2;%How precies measurement is. Lower number means more exacting values, could lead to repeated failures
...
p.optimizationRFStatus = 'off'; %'off', '1
% on', or 'con' 1

p.timePerOpimizationPoint = .5; %Duration of each data point during optimization
p.timeBetweenOptimizations = 240; %Seconds between optimizations (Inf to disable, 0 for optimization after every point)
