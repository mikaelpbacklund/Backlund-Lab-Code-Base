...
%% User Inputs
p.scanBounds = {[-750 -700],[720 770]};
p.scanAxes = {'x','y'};
p.scanStepSize = {2,2};
p.collectionType = 'counter';%analog or counter

%General
p.contrastVSReference = 'con';%'ref' or 'con'. If con, applies ODMR sequence but shows ref and con; if ref, uses fast sequence and only shows ref
...
p.forcedDelayTime = 0.125; %Time to force pause before (1/2) and after (full) collecting data
p.nDataPointDeviationTolerance = .00015;%How precies measurement is. Lower number means more exacting values, could lead to repeated failures
...
p.plotAverageSNR = true;%Only applicable if contrast enabled
