...
p.RFResonanceFrequency = 2.118; %on axis
%3.624
%General
p.RFAmplitude = 10;
...
p.AOMCompensation = 50;%How long AOM should be on before DAQ (negative flips to DAQ first)
...
p.nonInteractive = false;

%Saving
p.saveName = "1142_323RabiQuickTest";   % optional but recommended
p.saveDirectory = "C:\Users\User\Desktop\github repositories\Backlund-Lab-Code-Base\Saved Data"; % optional
p.autoSaveEveryItera1tion = true; % default
...
p.optimizationSteps = {-1:0.1:1}; %Locations the stage will move relative to current location
p.optimizationRFStatus = 'con'; %'off', 'on', 'snr', or 'con'
p.timePerOpimizationPoint = 5; %Duration of each data point during optimization
p.timeBetweenOptimizations = 3600; %Seconds between optimizations (Inf to disable, 0 for optimization after every point)
...
%reference value is lower than X percent of that post-opti;mization value, a new optimization will be performed. This
