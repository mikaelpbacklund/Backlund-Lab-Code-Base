%Scan duration cannot be larger than τ

%% Begin User Edit
%Settings
RFFrequency = 2.87;
piDuration = 50;
tauDuration = 200;%Tau that produces high contrast but is long enough to cover scan duration
scannedBufferLocation = 'before';%before, after, or both
scanStartDuration = 10;
scanEndDuration = 90;%Cannot be larger than tauDuration (-static buffer or divided by 2)
scanNSteps = 0;%Overrides step size. Set to 0 to use step size
scanStepSize = 2;
nIterations = 1;

%System settings
IQBuffer = [20 50];%Static buffer, will be partially overwritten by scan. Must be two numbers, can be 0
...
if strcmpi(scannedBufferLocation,'both')
    scanTitle = 'I/Q buffer duration before and after RF pulse'; %#ok<*UNRCH>
else
    scanTitle = sprintf('I/Q buffer duration %s RF pulse',scannedBufferLocation);
end
switch lower(scannedBufferLocation) %marks scan duration
    case 'before'
        IQBuffer(1) = 99;
    case 'after'
        IQBuffer(2) = 99;
    case 'both'        
        IQBuffer(1:2) = 99;
    otherwise
        error('scannedBufferLocation must be before, after, or both')
end
...
pulseInfo.duration = tauDuration;
pulseInfo.notes = 'τ';
instr{3} = finalizePulse(instr{3},pulseInfo,ii);
...
%Subtracts I/Q buffer from τ. Not useful for this script, but generally
%useful
% toModify = findPulses(instr{3},'notes','τ','contains');
% modifierAddresses = findPulses(instr{3},'notes','I/Q buffer','matches');
% newDurations = calculatePulseModifications(instr{3},toModify,modifierAddresses);
% for ii = 1:numel(newDurations)
%     instr{3}.userSequence(toModify(ii)).duration = newDurations(ii);
% end
...
piPulses = findPulses(instr{3},'activeChannels',{'RF'},'contains');

%Depending on which buffer is being scanned, identify all pi pulses and
%either subtract 1 or add 1 for before and after respectively (or do both)
switch lower(scannedBufferLocation)
    case 'before'
        scan.address = piPulses - 1;
    case 'after'
        scan.address = piPulses + 1;
    case 'both'
        scan.address = sort([(piPulses - 1) (piPulses + 1)],'ascend');
    otherwise
        error('scannedBufferLocation must be before, after, or both')
end
...
for ii = 1:numel(scan.address)
   scan.bounds{ii} = [scanStartDuration scanEndDuration];
end

tauPulses = findPulses(instr{3},'notes','τ','contains');
IQPulses = findPulses(instr{3},'notes','I/Q buffer','matches');

%Mimic scan to find tau pulse duration bounds
for ii = 1:numel(scan.address)
    instr{3}.userSequence(scan.address(ii)).duration = scanStartDuration;
end
tauPulseStartDurations = calculatePulseModifications(instr{3},tauPulses,IQPulses);

for ii = 1:numel(scan.address)
    instr{3}.userSequence(scan.address(ii)).duration = scanEndDuration;
end
tauPulseEndDurations = calculatePulseModifications(instr{3},tauPulses,IQPulses);

%Add tau pulses as scanned addresses then add their bounds
scan.address(end+1:end+numel(tauPulses)) = tauPulses;
for ii = 1:numel(tauPulses)
    scan.bounds{end+1} = [tauPulseStartDurations(ii) tauPulseEndDurations(ii)];
end
