   ...
   methods
      ...
      function h = connect(h)
         ...
         for currentConnectionAttempt = 1:h.maxConnectionAttempts
            ...
            if ~isfield(h.controllerInfo,'serialNumber') || any(isempty({h.controllerInfo.serialNumber}))
               if currentConnectionAttempt == h.maxConnectionAttempts
                  error('Unable to connect to all stages after %d attempts',h.maxConnectionAttempts)
               else
                  warning('Not all specified stage controllers were connected (%d times). Retrying...',currentConnectionAttempt)
      ...
      function h = relativeMove(h,spatialAxis,targetMovement,varargin)
         ...
         elseif (targetMovement > 0 && inPositiveBound) || (targetMovement < 0 && inNegativeBound)            
            ...
            %reset
            [h,targetMovement] = fineReset(h,spatialAxis,resetLocation,targetMovement);
