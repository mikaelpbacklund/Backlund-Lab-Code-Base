function ex = Copy_of_Random_Scanning_Using_Rabi(ex,p)

requiredParams = {'scanBounds','scanStepSize','collectionType','pulseNotes'};
...
[sentParams,~] = Copy_of_Rabi_template([],[]);
...
[ex.pulseBlaster,scanInfo] = Copy_of_Rabi_template(ex.pulseBlaster,sentParams);
...
scanInfo.bounds = cell(1,1);
[scanInfo.bounds{:}] = deal(p.scanBounds);
scanInfo.stepSize = ones(1,1);
scanInfo.stepSize(:) = p.scanStepSize;
scanInfo.bounds = {p.scanBounds};
scanInfo.stepSize = p.scanStepSize;
scanInfo.nSteps = ((p.scanBounds(2)-p.scanBounds(1))/p.scanStepSize)+1;
