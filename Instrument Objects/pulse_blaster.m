classdef pulse_blaster < instrumentType
    ...
    properties
        ...
        nChannels = 6;%Default is to use 6 channels
    ...
    methods
        ...
        function h = addPulse(h,pulseInfo,varargin)
            ...
            %will be inserted
            if nargin > 2
                if ~isa(varargin{1},'double'),   error('Pulse insertion location (third argument) must be a double'),    end
                if varargin{1} > numel(h.userSequence)
                    printOut(h,sprintf('Sequence is not %d pulses long, adding pulse to the end of the sequence instead',varargin{1}))
                    if isempty(h.userSequence)
                        h.userSequence = pulseInfo;
                    else
                        h.userSequence(end+1) = pulseInfo;
                    end
                else
                    h.userSequence(varargin{1}+1:end+1) = h.userSequence(varargin{1}:end);
                    h.userSequence(varargin{1}) = pulseInfo;
                end
            else
                if isempty(h.userSequence)
                    h.userSequence = pulseInfo;
        ...
        function h = modifyPulse(h,addressNumber,fieldToModify,modifiedValue)
            ...
            if numel(h.userSequence) < addressNumber
                error('Sequence is only %d pulses long, cannot modify pulse %d',numel(h.userSequence),addressNumber)
            end
            ...
            switch lower(fieldToModify)
                ...
                case {'activechannels','channels','active','names','name'}%Active channels, match binary and output then reorder
                    ...
                    [h.userSequence(addressNumber).numericalOutput,h.userSequence(addressNumber).channelsBinary]= interpretPulseNames(h,modifiedValue);
                    [h.userSequence(addressNumber).activeChannels,~] = interpretPulseNames(h,h.userSequence(addressNumber).numericalOutput);
                ...
                case {'channelsbinary','binary','binarychannels'}%Convert binary to numerical output then find activeChannels and properly structured binary
                    ...
                    h.userSequence(addressNumber).numericalOutput = bin2dec(fliplr(modifiedValue));
                    [h.userSequence(addressNumber).activeChannels,h.userSequence(addressNumber).channelsBinary]= interpretPulseNames(h,modifiedValue);

                case {'numericaloutput','output','numerical','number','value'}%Output simple replace then match binary and channel names
                    h.userSequence(addressNumer).numericalOutput = modifiedValue;
                    [h.userSequence(addressNumber).activeChannels,h.userSequence(addressNumber).channelsBinary]= interpretPulseNames(h,modifiedValue);
                case {'duration','dur','time'}%Duration simple replacement
                    h.userSequence(addressNumber).duration = modifiedValue;

                case {'directiontype','direction','type'}%Duration intepret direction then replace
                    h.userSequence(addressNumber).directionType = interpretName(h,modifiedValue,'Direction');

                case {'contextinfo','context'}%Context info simple replacement
                    h.userSequence(addressNumber).contextInfo = modifiedValue;

                case {'notes','usernotes','description'}%Notes simple replacement
                    h.userSequence(addressNumber).notes = modifiedValue;
        ...
        function h = calculateDuration(h)
                ...
                if ~isempty(loopTracker)
                    ...
                    for jj = 1:numel(chunks)-1
                        ...
                        if ~isempty(interiorPulses)
                            ...
                            totalDataDuration = totalDataDuration + chunkDuration*dataRatio;
                        end
                    end

                else
                    totalDuration = sum(durationValues);
                    totalDataDuration = sum(durationValues(dataOn));
                end

                %Saves to object's properties based on whether it is the user
                %sequence or adjusted sequence
                if ii == 1
                    h.userSequenceDuration = totalDuration;
                    h.userSequenceDataDuration = totalDataDuration;
                else
                    h.adjustedSequenceDuration = totalDuration;
                    h.adjustedSequenceDataDuration = totalDataDuration;
                end
            end
        ...
        function durations = calculatePulseModifications(h,addressesToModify,modifierAddresses,varargin)
            ...
            if strcmpi(calculationLocation,'before') || strcmpi(calculationLocation,'both')
                for ii = 1:numel(durations)
                        ...
                        durations(ii) = calculateDuration(h,durations(ii),...
            ...
            function newDuration = calculateDuration(h,oldDuration,allAddresses,currentAddress,calculationMethod)
